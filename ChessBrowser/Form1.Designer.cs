...
namespace ChessBrowser
{
    partial class Form1
    {
        ...
        private void UploadGamesToDatabase(string PGNfilename)
        {
            ...
            List<ChessGame> games = reader.ReadFile(PGNfilename);
            ...
            using (MySqlConnection conn = new MySqlConnection(connection))
            {
                try
                {
                    ...
                    {
                        MySqlCommand whitePlayerInsert = new MySqlCommand("insert ignore into Players (Name, Elo) values (@val1, @val2) ON DUPLICATE KEY UPDATE Elo = @val2; select pID from Players where Name = @val1", conn);
                        MySqlCommand blackPlayerInsert = new MySqlCommand("insert ignore into Players (Name, Elo) values (@val1, @val2) ON DUPLICATE KEY UPDATE Elo = @val2; select pID from Players where Name = @val1", conn);
                        MySqlCommand eventInsert = new MySqlCommand("insert ignore into Events (Name, Site, Date) values (@val1, @val2, @val3); select eID from Events where Name = @val1", conn);
                        MySqlCommand gameInsert = new MySqlCommand("insert into Games (Round, Result, Moves, BlackPlayer, WhitePlayer, eID) values (@val1, @val2, @val3, @val4, @val5, @val6);", conn);

                        blackPlayerInsert.Parameters.AddWithValue("@val1", game.WhitePlayer.Name);
                        blackPlayerInsert.Parameters.AddWithValue("@val2", game.WhitePlayer.Elo);
                        int bpID = Convert.ToInt32(blackPlayerInsert.ExecuteScalar());

                        whitePlayerInsert.Parameters.AddWithValue("@val1", game.BlackPlayer.Name);
                        whitePlayerInsert.Parameters.AddWithValue("@val2", game.BlackPlayer.Elo);
                        int wpID = Convert.ToInt32(blackPlayerInsert.ExecuteScalar());

                        eventInsert.Parameters.AddWithValue("@val1", game.Event);
                        eventInsert.Parameters.AddWithValue("@val2", game.Site);
                        eventInsert.Parameters.AddWithValue("@val3", game.EventDate);
                        int eID = Convert.ToInt32(eventInsert.ExecuteScalar());

                        gameInsert.Parameters.AddWithValue("@val1", game.Round);
                        gameInsert.Parameters.AddWithValue("@val2", game.Result);
                        gameInsert.Parameters.AddWithValue("@val3", game.Moves);
                        gameInsert.Parameters.AddWithValue("@val4", bpID);
                        gameInsert.Parameters.AddWithValue("@val5", wpID);
                        gameInsert.Parameters.AddWithValue("@val6", eID);
                        gameInsert.ExecuteNonQuery();
